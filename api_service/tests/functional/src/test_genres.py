...
async def test_all_genres_pagination_size(create_list_genres, fastapi_client):
    """Тест правильность размера пагинаци"""
...
async def test_all_genres_pagination_page(create_list_genres, fastapi_client, es_client):
    """Тест правильность размера пагинаци"""
    genres = create_list_genres
    expected_structure = [
        uuid_to_str(GenreApiSchema.build_from_model(genre)).__dict__
    ...
    # res = await es_client.search(index='genres_test')
    ...
    full_response = await fastapi_client.get(f"/api_service/v1/genres/")
    for i in full_response.json():
        print(i)
    print('++++++'*15)
    for i in expected_structure:
        print(i)
    print('==='*20)
