...
from api.v1.schemas import PersonApiSchema, PersonFilmApiSchema
...
async def test_all_persons(create_list_persons, fastapi_client):
    ...
    expected_structure = [
        uuid_to_str(PersonApiSchema.build_from_model(person)).dict()
...
async def test_search_film(create_list_persons, fastapi_client):
    ...
    expected_structure = [uuid_to_str(PersonApiSchema.build_from_model(person_for_find)).dict()]
...
async def test_person_films_by_id(create_one_film, fastapi_client):
    ...
    expected_structure = [uuid_to_str(PersonFilmApiSchema.build_from_model(film)).dict()]
